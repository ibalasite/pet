...
>
> **Platform**: Web (HTML5 SPA — Vercel CDN). Target engine: Phaser.js 3 + React 18. Sprite resolution: 32×32 px base frame (sprite_resolution_px = 32).
>
...
| 1.0.9 | 2026-05-04 | Review Agent R14 | Fix RES-IMG-014 description inconsistency: description said "Primary AVIF target ≤ 150 KB; WebP fallback ≤ 200 KB" (describing both formats' budgets) — after R8 added RES-IMG-015 as a separate AVIF row, the WebP row's description was not updated; now reads "Full arena race backdrop WebP fallback (VDD §4.3 + §7.5). Served when AVIF is not supported." to match the consistent pattern of RES-IMG-016 (sumo WebP) |
| 1.0.10 | 2026-05-04 | Review Agent R15 | Fix RES-IMG-042 description inconsistency: description said "AVIF primary (≤150 KB), WebP fallback ≤200 KB" (describing both formats' budgets in the AVIF-only row) — R14 established the pattern that each format row describes only its own format; the OG default AVIF row was not updated when this pattern was applied; now reads "AVIF primary format (VDD §7.5). Served preferentially over WebP for supported browsers." to match the consistent pattern of RES-IMG-044 (battle result AVIF primary) |
| 1.0.11 | 2026-05-04 | Review Agent R16 | Fix RES-IMG-043 description incomplete: missing "Served when AVIF is not supported." — all other WebP fallback rows (RES-IMG-014, RES-IMG-016) include a serving-context statement; RES-IMG-043 was the only WebP row without it; fix RES-IMG-044 description incomplete: ended with "AVIF primary format." — missing "(VDD §7.5)" citation and "Served preferentially over WebP for supported browsers." required by the consistent AVIF-row pattern established in R15 (RES-IMG-042) |
| 1.0.12 | 2026-05-04 | Review Agent R17 | Fix RES-IMG-016 description inconsistent serving-context statement: said "WebP fallback for supported browsers without AVIF." instead of the canonical pattern "Served when AVIF is not supported." used by all other WebP rows (RES-IMG-014, RES-IMG-043); fix RES-IMG-045 description missing serving-context statement: said "Required WebP fallback per VDD §7.5 (AVIF primary + WebP fallback for OG social card backgrounds)." — did not include "Served when AVIF is not supported." required by the consistent WebP-row pattern |
...
| RES-IMG-006 | pet-neglected-overlay-64.png | image | Stable Diffusion XL | pixel art wilted overlay for pet sprite, drooping accessories wilted state, 64x64px, transparent except overlay elements, muted desaturated grey tones, hard pixel edges, 8-bit style | 64×64px | ≤ 10 KB | needed | public/assets/sprites/reference/pet-neglected-overlay-64.png | Neglected state wilted accessory frame overlay (VDD §4.2). Applied on top of base sprite after (training_neglect_threshold_days = 3) days without training. Combined with CSS `filter: grayscale(60%) brightness(0.8)`. |
...
| ID | filename | type | source_tool | prompt | dimensions | file_size_budget | status | output_path | description |
|----|----------|------|-------------|--------|-----------|-----------------|--------|-------------|-------------|
| RES-IMG-013 | bg-arena-track-32x8.png | image | Aseprite | pixel art repeating track tile pattern, dark pixel grid 32x8px, dark charcoal `#2d2d5a` base with subtle lane markings, hard pixel edges, game arena floor tile for CSS background-repeat | 32×8px | ≤ 2 KB | needed | public/assets/backgrounds/bg-arena-track-32x8.png | Arena race track tile (VDD §4.3 + §7.5). Dark track tile pattern — repeating 16×8px conceptually, but delivered as 32×8 for pixel doubling. CSS: `background-repeat: repeat-x` on `--color-surface-overlay`. |
| RES-IMG-014 | bg-arena-race.webp | image | Midjourney v6 | pixel art arena racing background, dark navy `#1a1a2e` base, retro game race track scene, checkered finish line alternating 4x4px squares, lane dividers 2px dashed, side audience stands pixel art style, atmospheric dark with minimal color, 8-bit retro game art style, wide cinematic view, no antialiasing | 1920×400px | ≤ 200 KB | needed | public/assets/backgrounds/bg-arena-race.webp | Full arena race backdrop WebP fallback (VDD §4.3 + §7.5). Served when AVIF is not supported. Used in ArenaScene Phaser background. |
...
| RES-IMG-016 | bg-arena-sumo.webp | image | Midjourney v6 | pixel art sumo arena background, dark navy `#1a1a2e` base, circular pixel ring with gold `#fdcb6e` border 4px wide, sumo ring interior `#242444` raised surface, darker push-out zone at ring edge, retro 8-bit game art, top-down slightly isometric view, atmospheric dark scene | 1920×400px | ≤ 200 KB | needed | public/assets/backgrounds/bg-arena-sumo.webp | Sumo arena backdrop WebP fallback (VDD §4.3 + §7.5 — behind FF_ARENA_SUMO feature flag). Served when AVIF is not supported. |
| RES-IMG-017 | bg-arena-sumo.avif | image | Midjourney v6 | pixel art sumo arena background, dark navy `#1a1a2e` base, circular pixel ring with gold `#fdcb6e` border 4px wide, sumo ring interior `#242444` raised surface, darker push-out zone at ring edge, retro 8-bit game art, top-down slightly isometric view, atmospheric dark scene | 1920×400px | ≤ 150 KB | needed | public/assets/backgrounds/bg-arena-sumo.avif | Sumo arena backdrop AVIF primary format (VDD §7.5). VDD §7.5 requires AVIF primary + WebP fallback for all full-width arena backgrounds. Served preferentially over WebP for supported browsers. Behind FF_ARENA_SUMO feature flag. |
| RES-IMG-018 | bg-page-pattern.svg | image | Aseprite / hand-crafted | subtle pixel art dot grid tileable pattern, 16x16px tile, dark navy on deep navy `#1a1a2e`, very low opacity dots, retro CRT scanline texture feel, pure SVG geometric, no raster content | 16×16px tile (SVG, tileable) | ≤ 10 KB | needed | public/assets/backgrounds/bg-page-pattern.svg | Subtle page background texture (VDD §7.5). Applied as optional texture on base surfaces. SVG primary; PNG fallback ≤ 20 KB. |
| RES-IMG-019 | bg-page-pattern-fallback.png | image | Aseprite | subtle pixel art dot grid tileable pattern, 16x16px tile, dark navy on deep navy `#1a1a2e`, very low opacity, retro CRT feel, rasterized fallback | 16×16px tile (PNG tileable) | ≤ 20 KB | needed | public/assets/backgrounds/bg-page-pattern-fallback.png | PNG fallback for bg-page-pattern.svg (VDD §7.5). For browsers where SVG tiling causes rendering issues. |
...
| ID | filename | type | source_tool | prompt | dimensions | file_size_budget | status | output_path | description |
|----|----------|------|-------------|--------|-----------|-----------------|--------|-------------|-------------|
| RES-IMG-032 | logo-primary-dark.svg | image | Aseprite / hand-crafted SVG | pixel art wordmark "pixel-pet-arena" in Press Start 2P font style, dark background variant, near-white `#e8e8f0` color, 160x32px, hard pixel edges, grid-aligned letterforms, pure SVG vector | 160×32px (SVG) | ≤ 8 KB | needed | public/assets/brand/logo-primary-dark.svg | Primary logo — dark surfaces. Nav bar logo (160×32px). VDD §3.4 + §7.4. SVG primary; SVGO optimized. |
| RES-IMG-033 | logo-primary-dark.png | image | Aseprite / hand-crafted PNG export | pixel art wordmark "pixel-pet-arena", Press Start 2P style, near-white `#e8e8f0`, dark background, 320x64px @2x, hard pixel edges, PNG-24 with alpha | 320×64px @2x | ≤ 50 KB | needed | public/assets/brand/logo-primary-dark.png | PNG @2x export of primary logo (VDD §7.4). Used where SVG rendering is unreliable. |
| RES-IMG-034 | logo-inverted.svg | image | Aseprite / hand-crafted SVG | pixel art wordmark "pixel-pet-arena" in Press Start 2P font style, light background variant, dark navy `#1a1a2e` color, 160x32px, hard pixel edges, grid-aligned letterforms | 160×32px (SVG) | ≤ 8 KB | needed | public/assets/brand/logo-inverted.svg | Inverted logo — light background contexts (VDD §7.4). Admin portal and print-adjacent uses. |
| RES-IMG-035 | logo-inverted.png | image | Aseprite / hand-crafted PNG export | pixel art wordmark, light background, dark navy `#1a1a2e`, 320x64px @2x, hard pixel edges | 320×64px @2x | ≤ 50 KB | needed | public/assets/brand/logo-inverted.png | PNG @2x export of inverted logo (VDD §7.4). |
| RES-IMG-036 | mark-32.png | image | Aseprite | pixel art pet icon mark, standalone pixel creature silhouette without wordmark, 32x32px monochrome mark, hard pixel edges, works at favicon size, pure black foreground on transparent | 32×32px | ≤ 8 KB | needed | public/assets/brand/mark-32.png | Standalone pixel pet mark (VDD §7.4). Used for favicon (32×32px), browser tab, PWA icon base. |
| RES-IMG-037 | mark-64.png | image | Aseprite | pixel art pet icon mark, standalone pixel creature silhouette, 64x64px, hard pixel edges, dark navy background `#1a1a2e`, gold `#fdcb6e` accent, social avatar scale | 64×64px | ≤ 15 KB | needed | public/assets/brand/mark-64.png | 64×64 standalone mark (VDD §7.4). Social avatar, app icon contexts. |
| RES-IMG-038 | mark-128.png | image | Aseprite | pixel art pet icon mark, standalone pixel creature, 128x128px, dark navy background `#1a1a2e`, gold `#fdcb6e` accent, app icon quality, hard pixel edges, detailed at 128px | 128×128px | ≤ 30 KB | needed | public/assets/brand/mark-128.png | 128×128 standalone mark (VDD §7.4). PWA icon, higher-resolution app icon. |
| RES-IMG-039 | favicon-32.ico | image | Aseprite | pixel art pet icon 32x32px, ICO format, multi-size (16x16, 32x32), dark background, recognizable at 16px tab size, hard pixel edges | 32×32px (ICO with 16×16 embedded) | ≤ 5 KB | needed | public/favicon.ico | Favicon ICO file. Includes 16×16 and 32×32 sizes. Served from `/public/favicon.ico`. |
| RES-IMG-040 | favicon-192.png | image | Aseprite | pixel art pet icon 192x192px PNG, dark navy background `#1a1a2e`, gold `#fdcb6e` pet mark, PWA icon, hard pixel edges, `image-rendering: pixelated` intended | 192×192px | ≤ 30 KB | needed | public/favicon-192.png | PWA icon 192×192px for `manifest.json`. |
| RES-IMG-041 | favicon-512.png | image | Aseprite | pixel art pet icon 512x512px PNG, dark navy background `#1a1a2e`, gold `#fdcb6e` pet mark, PWA splash icon, hard pixel edges | 512×512px | ≤ 80 KB | needed | public/favicon-512.png | PWA icon 512×512px for `manifest.json` and splash screen. |

### §1.6 OG / Social Card Assets (VDD §8.8 / §7.5 / §7.6)

| ID | filename | type | source_tool | prompt | dimensions | file_size_budget | status | output_path | description |
|----|----------|------|-------------|--------|-----------|-----------------|--------|-------------|-------------|
| RES-IMG-042 | og-default-1200x630.avif | image | Midjourney v6 | dark luxury pixel art game card, 1200x630px, deep navy `#1a1a2e` background, "pixel-pet-arena" wordmark in Press Start 2P font upper left, abstract pixel creature silhouette center, rarity color accents gold `#fdcb6e`, purple `#a29bfe`, teal `#4ecdc4`, retro game aesthetic, premium collectible game social share card, hard pixel art style | 1200×630px | ≤ 150 KB | needed | public/assets/og/og-default-1200x630.avif | Default OG card (no specific pet). Used for landing page, leaderboard, and generic social shares. AVIF primary format (VDD §7.5). Served preferentially over WebP for supported browsers. |
| RES-IMG-043 | og-default-1200x630.webp | image | Midjourney v6 | dark luxury pixel art game card, 1200x630px, deep navy `#1a1a2e` background, "pixel-pet-arena" wordmark in Press Start 2P font upper left, abstract pixel creature silhouette center, rarity color accents gold `#fdcb6e`, purple `#a29bfe`, teal `#4ecdc4`, retro game aesthetic, premium collectible game social share card, hard pixel art style | 1200×630px | ≤ 200 KB | needed | public/assets/og/og-default-1200x630.webp | Default OG card WebP fallback (VDD §7.5). Served when AVIF is not supported. |
...
>
> All Phaser sprite sheet animations use PNG sprite atlases loaded via `this.load.spritesheet()`. The 6 Phaser sprite sheet assets are derived from CLIENT_IMPL.md §5.1 Animation List (idle, bounce, training-effort, battle-run, victory-particles) and CLIENT_IMPL.md §5.3 ArenaBattleScene state machine (battle-victory — 4-frame victory sequence in the VICTORY state). Note: the neglected state (ANIM-04) is a CSS `filter` overlay — not a Phaser sprite sheet — and is therefore covered by RES-IMG-006 in §1.1, not listed here.
...
| RES-ANIM-005 | pet-victory-sheet-ref-128x32.png | animation | Aseprite | pixel art pet victory celebration sprite sheet, 4 frames horizontal, 32x32px per frame = 128x32px (sprite_resolution_px = 32), winner celebration jump or raise arms pose, joyful pixel animation, transparent background, hard pixel edges | 4f × 30fps (128×32px) | ≤ 15 KB | needed | public/assets/sprites/anim-ref/pet-victory-sheet-ref-128x32.png | Battle victory animation reference (CLIENT_IMPL §5.3 ArenaBattleScene VICTORY state). 4-frame victory sequence played when winner is determined in ArenaBattleScene. Followed by gold particle burst ANIM-09. |
...
- [x] §1 VDD Visual Assets: VDD.md §4.3 arena backgrounds (race + sumo tiles + full backdrop) covered (RES-IMG-013 to RES-IMG-017) — includes AVIF primary (RES-IMG-015 race, RES-IMG-017 sumo) + WebP fallback (RES-IMG-014 race, RES-IMG-016 sumo) per VDD §7.5
- [x] §1 VDD Visual Assets: VDD.md §7.5 page background pattern covered (RES-IMG-018, RES-IMG-019)
- [x] §1 VDD Visual Assets: VDD.md §8.8 OG social card template covered (RES-IMG-042 to RES-IMG-047) — AVIF primary + WebP fallback + PNG last resort per VDD §7.5
...
| RES-IMG-007 | pet-sheet-template-256x32.png | internal | N/A | §1.1 Sprite sheet template — hand-crafted in Aseprite |
| RES-IMG-008 | pet-run-sheet-template-256x32.png | internal | N/A | §1.1 Run cycle sheet template — hand-crafted in Aseprite |
...
| RES-IMG-013 | bg-arena-track-32x8.png | internal | N/A | §1.3 Arena track tile — hand-crafted in Aseprite |
...
| RES-IMG-016 | bg-arena-sumo.webp | AI-generated | https://docs.midjourney.com/docs/terms-of-service | §1.3 Arena sumo background WebP fallback — Midjourney v6 |
| RES-IMG-017 | bg-arena-sumo.avif | AI-generated | https://docs.midjourney.com/docs/terms-of-service | §1.3 Arena sumo background AVIF primary — Midjourney v6 (same source as RES-IMG-016, re-exported) |
| RES-IMG-018 | bg-page-pattern.svg | internal | N/A | §1.3 Page background texture — hand-crafted SVG |
| RES-IMG-019 | bg-page-pattern-fallback.png | internal | N/A | §1.3 Page background texture PNG fallback — hand-crafted in Aseprite |
| RES-IMG-020 | icon-arena-race-24.svg | internal | N/A | §1.4 Race icon — hand-crafted pixel-art SVG |
| RES-IMG-021 | icon-arena-sumo-24.svg | internal | N/A | §1.4 Sumo icon — hand-crafted pixel-art SVG |
| RES-IMG-022 | icon-rarity-common-16.svg | internal | N/A | §1.4 Common rarity icon — hand-crafted pixel-art SVG |
| RES-IMG-023 | icon-rarity-rare-16.svg | internal | N/A | §1.4 Rare rarity icon — hand-crafted pixel-art SVG |
| RES-IMG-024 | icon-rarity-epic-16.svg | internal | N/A | §1.4 Epic rarity icon — hand-crafted pixel-art SVG |
| RES-IMG-025 | icon-rarity-legendary-16.svg | internal | N/A | §1.4 Legendary rarity icon — hand-crafted pixel-art SVG |
| RES-IMG-026 | icon-training-run-24.svg | internal | N/A | §1.4 Run training icon — hand-crafted pixel-art SVG |
| RES-IMG-027 | icon-training-strength-24.svg | internal | N/A | §1.4 Strength training icon — hand-crafted pixel-art SVG |
| RES-IMG-028 | icon-training-stamina-24.svg | internal | N/A | §1.4 Stamina training icon — hand-crafted pixel-art SVG |
| RES-IMG-029 | icon-food-24.svg | internal | N/A | §1.4 Food icon — hand-crafted pixel-art SVG |
| RES-IMG-030 | icon-share-24.svg | internal | N/A | §1.4 Share icon — hand-crafted pixel-art SVG |
| RES-IMG-031 | icon-copy-24.svg | internal | N/A | §1.4 Copy icon — hand-crafted pixel-art SVG |
| RES-IMG-032 | logo-primary-dark.svg | internal | N/A | §1.5 Primary logo SVG — hand-crafted wordmark |
| RES-IMG-033 | logo-primary-dark.png | internal | N/A | §1.5 Primary logo PNG @2x — exported from logo SVG |
| RES-IMG-034 | logo-inverted.svg | internal | N/A | §1.5 Inverted logo SVG — hand-crafted wordmark |
| RES-IMG-035 | logo-inverted.png | internal | N/A | §1.5 Inverted logo PNG @2x — exported from logo SVG |
| RES-IMG-036 | mark-32.png | internal | N/A | §1.5 Pixel pet standalone mark 32px |
| RES-IMG-037 | mark-64.png | internal | N/A | §1.5 Pixel pet standalone mark 64px |
| RES-IMG-038 | mark-128.png | internal | N/A | §1.5 Pixel pet standalone mark 128px |
| RES-IMG-039 | favicon-32.ico | internal | N/A | §1.5 Favicon ICO — generated from mark-32.png |
| RES-IMG-040 | favicon-192.png | internal | N/A | §1.5 PWA icon 192px |
| RES-IMG-041 | favicon-512.png | internal | N/A | §1.5 PWA icon 512px |
| RES-IMG-042 | og-default-1200x630.avif | AI-generated | https://docs.midjourney.com/docs/terms-of-service | §1.6 Default OG card AVIF — Midjourney v6 |
| RES-IMG-043 | og-default-1200x630.webp | AI-generated | https://docs.midjourney.com/docs/terms-of-service | §1.6 Default OG card WebP — Midjourney v6 |
| RES-IMG-044 | og-battle-result-1200x630.avif | AI-generated | https://docs.midjourney.com/docs/terms-of-service | §1.6 Battle result OG card template AVIF — Midjourney v6 |
| RES-IMG-045 | og-battle-result-1200x630.webp | AI-generated | https://docs.midjourney.com/docs/terms-of-service | §1.6 Battle result OG card template WebP fallback — Midjourney v6 |
| RES-IMG-046 | og-default-1200x630.png | AI-generated | https://docs.midjourney.com/docs/terms-of-service | §1.6 Default OG card PNG last-resort fallback — Midjourney v6 (same source as RES-IMG-042, re-exported) |
