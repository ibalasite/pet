...
| RES-IMG-008 | pet-run-sheet-template-128x32.png | image | Aseprite | pixel art run cycle sprite sheet template, 4 animation frames horizontal row, 32x32px per frame (sprite_resolution_px = 32), total 128x32px, running motion 4-frame loop, creature placeholder, transparent background, 8-bit retro game, hard pixel edges | 128×32px | ≤ 15 KB | needed | public/assets/sprites/reference/pet-run-sheet-template-128x32.png | Arena race run cycle sprite sheet: 4 run frames × 32px = 128×32px (sprite_resolution_px = 32). Used in ArenaBattleScene ANIM-08 battle race cycle. |
...
| RES-ANIM-004 | pet-run-sheet-ref-128x32.png | animation | Aseprite | pixel art pet run cycle sprite sheet, 4 frames horizontal single row, 32x32px per frame = 128x32px (sprite_resolution_px = 32), forward running motion 4-frame loop, transparent background, hard pixel edges, arena battle race animation | 4f × 30fps (128×32px) | ≤ 15 KB | needed | public/assets/sprites/anim-ref/pet-run-sheet-ref-128x32.png | Arena battle race run cycle reference (CLIENT_IMPL ANIM-08). 4-frame run loop sheet; loop repeated in Phaser for 5–15s arena match duration (arena_match_duration_min_seconds = 5, arena_match_duration_max_seconds = 15). Used in ArenaBattleScene. |
...
| RES-IMG-008 | pet-run-sheet-template-128x32.png | internal | N/A | §1.1 Run cycle sheet template — hand-crafted in Aseprite |
...
| RES-ANIM-004 | pet-run-sheet-ref-128x32.png | internal | N/A | §2 Arena run cycle reference sheet — hand-crafted in Aseprite |
