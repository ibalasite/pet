...
| RES-IMG-016 | bg-arena-sumo.webp | image | Midjourney v6 | pixel art sumo arena background, dark navy `#1a1a2e` base, circular pixel ring with gold `#fdcb6e` border 4px wide, sumo ring interior `#242444` raised surface, darker push-out zone at ring edge, retro 8-bit game art, top-down slightly isometric view, atmospheric dark scene | 1920×400px | ≤ 200 KB | needed | public/assets/backgrounds/bg-arena-sumo.webp | Sumo arena backdrop WebP fallback (VDD §4.3 + §7.5 — behind FF_ARENA_SUMO feature flag). WebP fallback for supported browsers without AVIF. |
