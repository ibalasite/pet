...
> Game UI icons: 24×24px custom pixel-art action icons + 16×16px rarity badge icons. SVG primary with `viewBox="0 0 24 24"` or `viewBox="0 0 16 16"` (VDD §7.2). Colors via `currentColor`. PNG fallback at corresponding pixel size. 12 custom pixel-art icons total: 8 at 24×24px for game interactions (RES-IMG-019–020, RES-IMG-025–030) and 4 at 16×16px for rarity badges (RES-IMG-021–024). Non-game navigation icons use Phosphor Icons v2.1 (Bold) — not listed here as they are an npm dependency, not a production asset.
