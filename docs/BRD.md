...
|------------|:----------:|-------------|---------|
| 隨機像素寵物展示 + 訪客即時互動 | **Must Have (P0)** | O1 | 零門檻是核心差異化；缺少此功能訪客無法體驗，認領漏斗無法啟動 |
| Email 認領流程（密碼信 + 專屬 URL）| **Must Have (P0)** | O1 | 身份識別核心機制；缺少此功能「持久化」不成立，整個產品無意義 |
| 基礎訓練 / 喂食系統 | **Must Have (P0)** | O1, O2 | 驅動 Day-7 回訪；缺少此功能玩家認領後沒有持續動力 |
| 競技場基礎對戰（跑步賽）| **Must Have (P0)** | O2 | 競技社交動力是留存核心；缺少此功能排行榜缺乏內容 |
| 全服排行榜 | **Must Have (P0)** | O2, O4 | 社交展示動力；缺少此功能競技無意義，口碑傳播缺少載體 |
| 戰績記錄頁面（可分享 URL）| **Must Have (P0)** | O4 | 高價值口碑傳播機制；§5.3 In Scope 明確列為 MVP Must Have；PRD §4.5 P0 升級確認 |
| 程序化像素寵物稀有度評分（Common/Rare/Epic/Legendary）| **Should Have (P1)** | O4 | 強化收藏驅動力；不阻擋上線但影響口碑傳播質量 |
| 第 2 種競技模式（相撲擂台）| **Could Have (P2)** | O2, O5 | 在第 1 種模式驗證後擴展，降低玩家疲勞 |
| 寵物交易市場 | **Won't Have (Out)** | O5 | 需 DAU > 1,000 才有成交量；MVP 先驗證留存 |
| 付費道具 / 稀有寵物直售 | **Won't Have (Out)** | O5 | 留存驗證前不引入付費摩擦 |
...
| **COPPA**（美國兒童線上隱私保護）| 未滿13歲 (under 13) 用戶數據 | 禁止收集未滿13歲 (under 13) 兒童個人資料（含 email）；需明確的年齡確認機制 | MVP 設計前 | Legal + Engineering |
| **台灣個資法**（若台灣用戶為目標市場）| 台灣用戶個人資料 | 個人資料蒐集需當事人同意；隱私權政策公開；用戶有權查閱、更正 | BRD 核准後 4 週內確認 | Legal |

> 若無法在上線前完成全部法規合規，優先處理 GDPR（影響 EU 用戶）和 COPPA（強制性，未滿13歲 (under 13) 排除機制）。其餘依實際市場進入時程補齊。
...
| COPPA — 未滿13歲 (under 13) 禁止 | 認領流程加入年齡確認選項（「我年滿 13 歲」checkbox）| 無重大架構影響；email 驗證邏輯需排除明確標記為兒童的請求 | 設計成本 +$500 |
