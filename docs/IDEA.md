...
**本 IDEA 的分類**：Sustaining + Adjacent（持續性兼鄰近式創新）

**分類依據**：在既有虛擬寵物遊戲市場中，透過「無帳號門檻 email 認領制」與「程序化像素藝術生成」提供更低使用門檻的替代方案（Sustaining）；同時將傳統桌面虛擬寵物玩家群體延伸至「不願創帳號的輕度休閒瀏覽器用戶」這一新市場（Adjacent）。

*分類影響資源投入策略：Sustaining 適合快速 ROI 路徑，Adjacent 需驗證新用戶群的行為模式。*
...
| **推估依據** | 參考競品：類 Tamagotchi 瀏覽器遊戲月活約 3-10 萬（DAU/MAU ≈ 5-10%），啟動期保守估 DAU 2k-5k；競技場活動期峰值 PCU 約 DAU 10% |
