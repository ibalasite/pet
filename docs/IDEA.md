...
### 3.3 Persona Cards（代表性使用者畫像）

> 以下 Persona 代表主要目標使用者群，供 BRD Persona 章節直接引用。每張卡片對應一類核心用戶行為模式。

---

**Persona 1 — Alex（Casual Gamer）**

| 欄位 | 內容 |
|------|------|
| **Name / Age / Role** | Alex，24 歲，大學畢業生，行銷助理 |
| **Goals** | 午休或通勤時有輕鬆的短暫娛樂；不想填寫帳號就能立刻開始遊戲；擁有一隻「只屬於自己」的獨特虛擬寵物 |
| **Frustrations** | 大多數遊戲要求完整帳號註冊，心理門檻過高；LocalStorage 寵物資料常因換裝置或清除快取而消失 |
| **How Pixel Pet Arena helps** | 訪客模式零門檻即時互動，確認喜歡後只需輸入 email 即可永久認領，隨時從任何裝置回訪同一隻寵物。 |

---

**Persona 2 — Maya（Content Creator）**

| 欄位 | 內容 |
|------|------|
| **Name / Age / Role** | Maya，28 歲，自由創作者，YouTuber / 像素藝術 Streamer |
| **Goals** | 在直播或社群媒體上展示獨特的像素寵物吸引觀眾；分享戰績截圖引發互動話題；收藏稀有外型寵物作為頻道特色元素 |
| **Frustrations** | 現有像素藝術遊戲缺乏社交競技維度，無法在社群媒體製造話題；公開排行榜和分享功能通常缺乏可客製化的展示頁面 |
| **How Pixel Pet Arena helps** | 公開可分享的寵物戰績 URL + 全服排行榜，讓 Maya 能直接把寵物頁面嵌入社群媒體或直播畫面。 |

---

**Persona 3 — Jordan（Competitive Player）**

| 欄位 | 內容 |
|------|------|
| **Name / Age / Role** | Jordan，20 歲，大學生，電競社團成員 |
| **Goals** | 在競技場排行榜登頂，打敗其他玩家的寵物；透過訓練和特殊食物系統性提升寵物屬性；保持競技場 Top 10 排名 |
| **Frustrations** | 輕量 HTML5 遊戲通常缺乏真正的 PvP 競技機制和留存動力；排行榜容易被機器人刷榜破壞公平性 |
| **How Pixel Pet Arena helps** | 多模式競技場（跑步賽 / 相撲擂台）+ 反作弊機制 + 全服排行榜，提供可持續的競技動力與成就感。 |

---

**Persona 4 — Sam（NFT Skeptic / Collector）**

| 欄位 | 內容 |
|------|------|
| **Name / Age / Role** | Sam，32 歲，軟體工程師，對區塊鏈持觀望態度的收藏愛好者 |
| **Goals** | 擁有真正稀有且唯一的虛擬寵物（但不想碰加密錢包）；在寵物交易市場以合理價格買賣稀有外型；長期持有稀有寵物作為數字收藏品 |
| **Frustrations** | P2E 鏈遊需要 Gas Fee 且市場受加密週期影響大；NFT 的「稀有性」常被質疑，且門檻對非幣圈用戶過高 |
| **How Pixel Pet Arena helps** | 程序化生成保證每隻寵物 >10 億種組合唯一性，email 認領即所有權，P2P 交易市場無需錢包或 Gas。 |

---

### 3.4 非目標使用者（Not Our Users）
...
| ❌ 未滿13歲兒童（under 13）（非目標族群） | Email 認領機制需要基本 email 使用能力；COPPA 法規要求未滿13歲（under 13）兒童數據特殊處理，MVP 階段暫不優化此群體 |
...
| 基礎訓練 / 喂食系統（提升屬性值）| 玩家願意每日回訪進行養成操作 | Day-7 留存率 ≥ 25%（MVP Go Threshold，operational gate；aspirational target：>30%，見 §5.2）|
...
| **Auth Service（Email OTP + URL Token）** | 處理 email 認領請求、生成一次性6位數密碼 + 專屬 URL token、驗證回訪身份 | Node.js + JWT（短效 token）+ Redis（token 黑名單）|
...
| R7 | GDPR / CAN-SPAM 合規：EU 用戶 email 認領機制可能違反 GDPR 數據最小化原則；CAN-SPAM 要求 transactional email 明確標示；COPPA 對未滿13歲（under 13）兒童數據保護有強制要求 | 法規 | MEDIUM | HIGH | 🔴 HIGH | BRD 前完成法務審查 email 認領機制（含數據保留政策 + 刪除流程）；在 §3.4 明確排除未滿13歲兒童；隱私政策中載明 email 用途僅限認領與通知 |
