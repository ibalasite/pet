Feature: Suspicious Battle Detection (US-ADMIN-005)
  ...
  Scenario: Pet is auto-flagged SUSPICIOUS after exceeding the battle threshold in the detection window
    Given pet "spammer-token-001" has completed (bot_detection_battles_threshold = 50) battles within the last (bot_detection_window_minutes = 60) minutes
  ...
  Scenario: Moderator reviews a SUSPICIOUS pet and submits an arena ban with a reason
    ...
    When the moderator submits a ban action via POST /admin/pets/spammer-token-001/ban with reason "Automated rapid-fire battles consistent with scripted bot behaviour"
  ...
  Scenario: Banned pet cannot enter future arena matches
    ...
    When "banned-token-002" attempts to join the matchmaking queue via POST /api/v1/arena/join
