Feature: Arena UI — Enter Arena Flow, Rate Limit UI, and Battle Result Display (US-ARENA-001, US-ARENA-002)
  ...
  Scenario: Opponent found within matchmaking timeout — battle animation plays
    ...
    And an opponent is found and POST /api/v1/arena/enter responds HTTP 200 with "matchId", "result", and "opponentPetId" before (arena_matchmaking_timeout_seconds = 30) seconds
    When the ArenaPage receives the successful match response
  ...
  Scenario: No opponent found — AIOfferModal is shown after 30-second timeout
    Given the matchmaking service has no available opponent and will return HTTP 408 "MATCHMAKING_TIMEOUT" after (arena_matchmaking_timeout_seconds = 30) seconds
